  ...
  const testRows = useMemo(() => {
    if (concept && concept.setMembers.length > 0) {
      return concept?.setMembers.map((memberConcept) => ({
        ...
        testType: (
          <div className={styles.testType}>{memberConcept.display}</div>
        ),
        result: isLoadingResult ? (
          <SkeletonText />
        ) : (
          testResultObs?.groupMembers?.find(
            (obs) => obs.concept.uuid === memberConcept.uuid
          )?.value.display ?? "--"
        ),
        normalRange:
